...
const META = {
  playerDeck: () => null,   // return an array of 30 card ids, or null for default
...
function newSide(deckList){
  ...
  return {life:START_LIFE, soul:START_SOUL, soulBroken:false, essence:0, maxEss:0, deadEss:0, deck, hero:null,
          hand:[], deadHand:[], living:[], dead:[], sits:{living:[], dead:[]}, sacrificed:false};
}

function newGame(playerHeroId){
  state = {player:newSide(META.playerDeck()), ai:newSide(), turn:"player", round:1,
           winner:null, mode:null, log:[], phase: playerHeroId ? "play" : "heroSelect"};
  const heroIds = Object.keys(HEROES);
  state.ai.hero = heroIds[Math.floor(Math.random()*heroIds.length)];
  draw("player",5); draw("ai",5);
  log("A new age begins. You go first (no draw on turn 1).","s");
  if(playerHeroId){ state.player.hero = playerHeroId; beginPlayerTurn(); }
}
...
function enterDeadField(k, unit){
  const s = S(k);
  const bonus = (s.hero === "morwen" ? 1 : 0) + sumSitFx(s, "dead", "enterDeadHp");
  unit.hp += bonus; unit.maxHp += bonus;
  s.dead.push(unit);
}
...
function makeUnit(cardId, dim){
  const def = DEFS[cardId], st = dim === "living" ? def.l : def.d;
  return {uid:++uidSeq, cardId, dim, atk:st[0], hp:st[1], maxHp:st[1], exhausted:true};
}
...
function destroySituation(uid){
  ...
  if(f.dim === "living"){
    ...
    if(s.deadHand.length < MAX_HAND){
      s.deadHand.push(f.sit.cardId);
...
function sitFxOf(sit){ return faceOf(sit.cardId, sit.dim === "dead").sitFx || null; }
...
function sacrifice(k, handIdx){
  ...
  if(s.maxEss < ESS_CAP){
    s.maxEss++; s.essence++;
    log(who(k)+" sacrificed "+DEFS[cardId].name+" → dead hand. Essence is now "+s.maxEss+".");
  } else {
    log(who(k)+" sacrificed "+DEFS[cardId].name+" → dead hand. Essence is already at its peak.");
  }
...
function canPayBlood(s, def, fromDead){
  ...
  return fromDead ? true : s.life > def.blood; // never suicide outright; dead side may break its own soul
}
function payBlood(k, s, def, fromDead){
  if(fromDead && !s.soulBroken){ s.soul -= def.blood; log("Bloodprice: "+def.blood+" Soul.","s"); }
  else { s.life -= def.blood; log("Bloodprice: "+def.blood+" Life.","s"); }
  FX.damage({hero:k, dim: fromDead ? "dead" : "living"}, def.blood);
}

function playUnit(k, handIdx, fromDead){
  const s = S(k), hand = fromDead ? s.deadHand : s.hand;
  const cardId = hand[handIdx], def = DEFS[cardId];
  const dim = fromDead ? "dead" : "living";
  const cost = def.blood ? 0 : def.cost;
  if(cost > poolOf(s, fromDead) || s[dim].length >= MAX_FIELD || !canPayBlood(s, def, fromDead)) return false;
  hand.splice(handIdx,1);
  if(def.blood) payBlood(k, s, def, fromDead);
  else pay(s, cost, fromDead);
  const u = makeUnit(cardId, dim);
  ...
  log(who(k)+" played "+(dim === "dead" && def.deadName ? def.deadName : def.name)+" into the "+dim+" field.");
  ...
  if(def.ability === "spirit"){ // the Spirit is summoned into the OTHER dimension
    const other = dim === "living" ? "dead" : "living";
    if(s[other].length < MAX_FIELD){
      const sp = makeUnit("spirit", other);
      if(other === "dead") enterDeadField(k, sp); else s.living.push(sp);
      log((dim === "living" ? "Wraith Caller" : "Wraith Lord")+" sends a Spirit across the veil into the "+other+" field.","s");
    }
  }
  if(def.ability === "digger" || def.ability === "ferry"){
    ...
    if(s.deck.length){
      const c = s.deck.pop();
      s.deadHand.push(c);
      log(who(k)+"'s "+(dim === "dead" && def.deadName ? def.deadName : def.name)+" ferries "+DEFS[c].name+" into the dead hand.","s");
    }
  }
  ...
  if(def.ability === "twin" && dim === "living" && s.deadHand.length < MAX_HAND){
    s.deadHand.push(cardId);
...
const guardsOf = (k, dim) => S(k)[dim].filter(u => DEFS[u.cardId].ability === "guard");

function playSituation(k, handIdx, fromDead){
  const s = S(k), hand = fromDead ? s.deadHand : s.hand;
  const cardId = hand[handIdx], dim = fromDead ? "dead" : "living";
  const cost = DEFS[cardId].cost;
  if(cost > poolOf(s, fromDead) || s.sits[dim].length >= MAX_SITS) return false;
  if(s.sits[dim].some(x => x.cardId === cardId)) return false; // situations are unique per dimension
  hand.splice(handIdx,1);
  pay(s, cost, fromDead);
  s.sits[dim].push({uid:++uidSeq, cardId, dim});
  log(who(k)+" set "+faceOf(cardId, fromDead).name+" over the "+dim+" dimension.");
  return true;
}
...
function castSpell(k, fromDead, handIdx, target){ // target = {uid}
  ...
  if(cardId === "smite"){ if(t){ t.unit.hp -= 3; FX.damage({uid:t.unit.uid}, 3); } }
    ...
    if(t){
      s.dead = s.dead.filter(u => u.uid !== t.unit.uid);
      s.living.push(makeUnit(t.unit.cardId, "living"));
      log(DEFS[t.unit.cardId].name+" claws back into the world of the living!","s");
    }
  }
  ...
  if(!fromDead && face.echo && s.deadHand.length < MAX_HAND){ // Echo: the cast reverberates across
    s.deadHand.push(cardId);
...
function runSpellFx(k, fromDead, face, t){
  ...
  if(fx.kind === "dmg"){ if(t){ t.unit.hp -= fx.n; FX.damage({uid:t.unit.uid}, fx.n); } }
    ...
    if(s.deck.length){
      const c = s.deck.pop(), cd = DEFS[c], dim = fromDead ? "dead" : "living";
      if(cd.type === "unit" && !(fromDead && cd.deadForm === "situation") && s[dim].length < MAX_FIELD){
        const u2 = makeUnit(c, dim);
  ...
  else if(fx.kind === "possess"){ // the dead answer whoever calls loudest
    if(t && t.side === foe(k) && t.dim === "dead" && s.dead.length < MAX_FIELD){
      e.dead = e.dead.filter(x => x.uid !== t.unit.uid);
      t.unit.exhausted = true;
      s.dead.push(t.unit);
      log(unitName(t.unit)+" turns on its master and joins "+poss(k)+" dead legion!","s");
    }
  }
}
...
function attack(attackerUid, targetUid){ // targetUid null = hero
  ...
  if(targetUid != null && gs.length){
    ...
    if(!tt || DEFS[tt.unit.cardId].ability !== "guard") return; // must break the Guard first
  }
  ...
  if(targetUid == null){
    ...
    if(t.unit.hp > 0){
      ...
      const hv = DEFS[a.unit.cardId].harvest;
    ...
    for(const k of ["player","ai"]){
      ...
      for(const u of [...s.dead]){
        if(u.hp <= 0){
          ...
          if(def.ability === "plague"){
            log("Hollow Bearer bursts: 1 damage to all enemy dead units.","s");
            S(foe(k)).dead.forEach(x => x.hp -= 1);
          }
          if(def.ability === "undying" && s.deadHand.length < MAX_HAND){
            s.deadHand.push(u.cardId);
...
function handleLivingDeath(k, u){
  ...
  if(def.ability === "undying" && s.hand.length < MAX_HAND){
    s.hand.push(u.cardId);
    log(def.name+" refuses death and returns to "+poss(k)+" hand!","s");
  } else if(!def.token && s.deadHand.length < MAX_HAND){
    s.deadHand.push(u.cardId);
...
function checkWin(){
  ...
  // Soul 0 doesn't end the game — it tears the veil: the winner's dead units
  // gain the upper hand and attack the broken player's Life directly.
  for(const k of ["player","ai"]){
    const s = S(k);
    if(!s.soulBroken && s.soul <= 0){
      s.soul = 0; s.soulBroken = true;
      log("💀 "+(k === "player" ? "Your Soul is SHATTERED" : "The AI's Soul is SHATTERED")+"! The veil tears — "+poss(foe(k))+" dead units now strike Life directly.","s");
      FX.shatter(k);
    }
  }
...
function startTurn(k){
  ...
  s.living.forEach(u => u.exhausted = false);
  s.dead.forEach(u => u.exhausted = false);
...
function beginPlayerTurn(){
  ...
  startTurn("player");
  FX.banner("Your Turn");
  if(state.player.hand.length && !state.winner){
    state.mode = {type:"sacrificePrompt"};
    FX.hint("Sacrifice a card (+1 Essence, it joins your dead hand) — or skip.");
  } else {
    state.player.sacrificed = true;
    state.mode = null;
  }
}
...
/* ---- scripted heuristics ---- */
function scriptedSacrifice(){
  const s = state.ai;
  if(s.sacrificed || !s.hand.length) return false;
  if(!(s.maxEss < 7 || s.hand.length >= 6)) { s.sacrificed = true; return false; }
...
function scriptedPlayOnce(){
  const s = state.ai, p = state.player;
  const options = [];
  const sitOk = (dim, id) => s.sits[dim].length < MAX_SITS && !s.sits[dim].some(x => x.cardId === id);
  s.hand.forEach((id,i) => {
    const d = DEFS[id];
    if(d.cost > s.essence) return;
    if(d.type === "unit" && s.living.length < MAX_FIELD) options.push({fromDead:false,i,prio:d.cost});
    if(d.type === "situation" && sitOk("living", id)) options.push({fromDead:false,i,prio:d.cost+2});
  });
  s.deadHand.forEach((id,i) => {
    const d = DEFS[id];
    if(d.cost > s.deadEss) return;
    if(d.type === "unit"){
  ...
  if(options.length){
    ...
    return playCard("ai", options[0].i, options[0].fromDead);
  }
  const trySpell = (id, pick) => {
    for(const [fromDead, hand] of [[false,s.hand],[true,s.deadHand]]){
      const i = hand.indexOf(id);
      if(i < 0 || DEFS[id].cost > poolOf(s, fromDead)) continue;
      const t = pick(validTargets("ai", id, fromDead), fromDead);
      if(t){ castSpell("ai", fromDead, i, {uid:t.uid}); return true; }
    }
    return false;
  };
  const tryNoTarget = (id, cond) => {
    for(const [fromDead, hand] of [[false,s.hand],[true,s.deadHand]]){
      const i = hand.indexOf(id);
      if(i < 0 || DEFS[id].cost > poolOf(s, fromDead)) continue;
      if(cond(fromDead)){ castSpell("ai", fromDead, i, null); return true; }
    }
    return false;
  };
  return tryNoTarget("surge", fromDead => fromDead ? s.deadHand.length > 1 : s.hand.length > 1)
    ...
    || trySpell("smite", ts => ts.filter(u => findUnit(u.uid).side === "player" && u.hp <= 3 && usum(u) >= 5).sort((a,b)=>usum(b)-usum(a))[0])
    || trySpell("banish", ts => ts.filter(u => findUnit(u.uid).side === "player" && usum(u) >= 7).sort((a,b)=>usum(b)-usum(a))[0])
    || trySpell("haunt", (ts, fromDead) => ts.filter(u => findUnit(u.uid).side === "ai" && atkOf(u, s) >= 3 && (fromDead || u.hp <= 2)).sort((a,b)=>atkOf(b,s)-atkOf(a,s))[0])
    ...
    || trySpell("resurrection", ts => {
         const myPow = s.living.reduce((n,u)=>n+usum(u),0), pPow = p.living.reduce((n,u)=>n+usum(u),0);
         return (pPow > myPow) ? ts.filter(u => findUnit(u.uid).side === "ai" && lsum(u.cardId) >= 6).sort((a,b)=>lsum(b.cardId)-lsum(a.cardId))[0] : null;
       })
...
async function scriptedAttacks(){
  const s = state.ai, p = state.player;
  for(const dim of ["living","dead"]){
    ...
    while(ready().length && !state.winner){
      ...
      const allFace = !guards.length && ready().reduce((n,x)=>n+atkOf(x,s),0) >= heroHp;
      let targetUid = null;
      if(!allFace){
        const pow = atkOf(u,s);
        const freeKills = pool.filter(t => pow >= t.hp && atkOf(t,p) < u.hp).sort((a,b)=>usum(b)-usum(a));
      ...
      FX.render(); await FX.pause(320);
    }
  }
}

async function aiTurnScripted(){
  await FX.pause(500);
  if(scriptedSacrifice()){ FX.render(); await FX.pause(420); }
  while(!state.winner && scriptedPlayOnce()){ FX.render(); await FX.pause(420); }
  await scriptedAttacks();
}
...
function llmStateSummary(){
  ...
  me.deadHand = S("ai").deadHand.map((id,i) => ({index:i, name:faceOf(id,true).name, costsDeadEnergy:DEFS[id].cost, type:DEFS[id].type, text:faceOf(id,true).text}));
  const en = sideView("player");
  en.handCount = S("player").hand.length; en.deadHandCount = S("player").deadHand.length;
  me.champion = {name: HEROES[S("ai").hero].name, passive: HEROES[S("ai").hero].passive};
  en.champion = {name: HEROES[S("player").hero].name, passive: HEROES[S("player").hero].passive};
...
function llmLegalActions(){
  ...
  const addPlays = (hand, fromDead) => hand.forEach((id,i) => {
    const def = DEFS[id], cost = def.cost;
    if(cost > poolOf(s, fromDead)) return;
    const where = fromDead ? "dead" : "living";
    const asSituation = def.type === "situation" || (def.type === "unit" && fromDead && def.deadForm === "situation");
    if(def.type === "unit" && !asSituation && s[where].length < MAX_FIELD)
      acts.push({do:{action:"play", hand:where, index:i}, desc:"Play "+faceOf(id,fromDead).name+" into the "+where+" field (cost "+cost+")"});
    if(asSituation && s.sits[where].length < MAX_SITS && !s.sits[where].some(x => x.cardId === id))
...
function llmExecute(a){
  ...
  if(d.action === "skip_sacrifice"){ state.ai.sacrificed = true; log("AI forgoes the sacrifice.","a"); return true; }
